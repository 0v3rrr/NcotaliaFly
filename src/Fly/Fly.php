...
class Fly extends PluginBase implements Listener{
	...
	public function onCommand(CommandSender $sender, Command $command, $label, array $args) : bool{
		if (!$sender->hasPermission("fly.command")) {
			$sender->sendMessage(TextFormat::RED.TextFormat::ITALIC."Vous n'avez pas la permission!");
			return false;
		}
		$sender = $sender;
		if (isset($args[0])) {
		...
		if (!$this->flyAllowed($world)) {
			$sender->sendMessage(TextFormat::RED.TextFormat::ITALIC."Vous ne pouvez pas utliser cette commande dans ce monde");
			return false;
		}
